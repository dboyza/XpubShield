...
| Demo preview to beta readiness | Phase 9 | Partial | `README.md`, `docs/HANDOFF.md`, `docs/RELEASE_READINESS.md`, this audit | Baseline builds/tests pass, dependency audit is resolved, and missing primary pages are added, but beta blockers remain: partial persistence/analysis, broader fixtures, richer live history, background scan scheduling, and security review | Remediate confirmed blockers in milestone commits | `npm audit`, `npm run build`, `cargo test`, `npm run tauri -- build`, UI smoke test |

## Earliest Incomplete Work

The earliest incomplete remediation item is persistence and analysis completeness. Close gaps in this order: generic label persistence, backend scan enrichment, analysis hardening, fixture expansion, background scan scheduling, and final security review.
