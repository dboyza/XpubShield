...
The next incomplete Phase 4 roadmap item is real Rust-backed descriptor parsing and address derivation. The first persistence slice now stores the current wallet report and UTXO metadata locally without changing descriptor derivation, backend scanning, or PSBT parsing.
